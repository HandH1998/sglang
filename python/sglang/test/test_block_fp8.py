...
#         self.assertTrue(torch.allclose(out.to(torch.float32), ref_out.to(torch.float32), rtol=0.15))
...
#         self.assertTrue(torch.mean(torch.abs(out.to(torch.float32) - ref_out.to(torch.float32))) / torch.mean(
#         torch.abs(ref_out.to(torch.float32))) < 0.001)
...
def torch_w8a8_block_fp8_moe(a, w1, w2, w1_s, w2_s, score, topk, block_shape):
    ...
    for i in range(w1.shape[0]):
        mask = topk_ids == i
        if mask.sum():
            inter_out = native_w8a8_block_fp8_matmul(a_q[mask], w1[i], a_s[mask], w1_s[i], block_shape, output_dtype=a.dtype)
            act_out = SiluAndMul().forward_native(inter_out)
            act_out_q, act_out_s = native_per_token_group_quant_fp8(act_out, block_k)
            act_out = act_out.to(torch.float32)
            out[mask] = native_w8a8_block_fp8_matmul(act_out_q, w2[i], act_out_s, w2_s[i], block_shape, output_dtype=a.dtype)
...
class TestW8A8BlockFP8FusedMoE(unittest.TestCase):
    ...
    def _w8a8_block_fp8_fused_moe(self, M, N, K, E, topk, block_size, dtype, seed):
        ...
        w1_s = torch.rand((E, n_tiles_w1, k_tiles_w1), dtype=torch.float32) * factor_for_scale
        w2_s = torch.rand((E, n_tiles_w2, k_tiles_w2), dtype=torch.float32) * factor_for_scale

        score = torch.randn((M, E), dtype=dtype)

        with torch.inference_mode():
            out = fused_moe(a, w1, w2, score, topk, renormalize=False, use_fp8_w8a8=True, w1_scale=w1_s, w2_scale=w2_s, block_shape=block_size)
            print(out)
            # ref_out = torch_w8a8_block_fp8_moe(a, w1, w2, w1_s, w2_s, score, topk, block_size)

        # self.assertTrue(torch.mean(torch.abs(out.to(torch.float32) - ref_out.to(torch.float32))) / torch.mean(
        # torch.abs(ref_out.to(torch.float32))) < 0.02)
