...
from sglang.srt.layers.quantization.fp8_utils import input_to_float8, block_quant_to_tensor_quant
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        if not global_server_args_dict["disable_mla"]:
            for layer_id in range(self.config.num_hidden_layers):
                ...
                if hasattr(self.quant_config, "weight_block_size") and w.dtype == torch.float8_e4m3fn:
                    weight_block_size = self.quant_config.weight_block_size
                    if weight_block_size is not None:
                        assert hasattr(self_attn.kv_b_proj, "weight_scale_inv")
                        w, scale = block_quant_to_tensor_quant(w, self_attn.kv_b_proj.weight_scale_inv, weight_block_size)
