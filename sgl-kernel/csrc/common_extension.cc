...
//   m.impl("top_k_top_p_sampling_from_probs", torch::kCUDA, &top_k_top_p_sampling_from_probs);
  ...
  m.def("gemm_forward_cuda(Tensor _in_feats, Tensor _kernel, Tensor _wscales, Tensor _ascales, Tensor _w_szs, Tensor _a_ssums, Tensor _out_feats) -> ()");
  m.impl("gemm_forward_cuda", torch::kCUDA, &gemm_forward_cuda);
// }
}
